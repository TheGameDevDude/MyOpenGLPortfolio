...
void main() {
	...
	vec3 reflColor = texture(material.texture_reflection1, fragTextureCoords).rgb;

	float ratio = 1.00 / 1.52;
	vec3 I = normalize(fragPos - camaraPosition);
	vec3 R = refract(I, normalize(fragNormals), ratio);

	vec3 color = mix(texture(reflectionCube, R).rgb * reflColor, result, 0.6f);
